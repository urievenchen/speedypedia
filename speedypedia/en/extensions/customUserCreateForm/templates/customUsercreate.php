...
class customUsercreateTemplate extends BaseTemplate {
	/**
	...
	/**
	 *	Creates a Radio button input with each of the options
	 *  $name - name of input
	 *  $options - array of radio button options.
	 */
	function addRadioButtons( $name, $msg, $options, $helptext=false ) {
		$this->data['extraInput'][] = array (
			'name' => $name,
			'type' => 'radio',
			'options' => $options,
		...
		<form name="userlogin2" id="userlogin2" class="mw-ui-vform" method="post" action="<?php $this->text( 'action' ); ?>">
			...
			if ( isset( $this->data['extraInput'] ) && is_array( $this->data['extraInput'] ) ) { ?>
				<?php foreach ( $this->data['extraInput'] as $inputItem ) { ?>
					<div class="mw-ui-vform-field">
						<label for="wpGender">How do you prefer to be described? (optional)</label>
						<?php
						...
						?>
							<div class="">
							<?php
								foreach ( $inputItem['options'] as $radioChoice ) {
							?>		
									<input 
										type="radio"
										name="<?php echo htmlspecialchars( $inputItem['name'] ); ?>"
